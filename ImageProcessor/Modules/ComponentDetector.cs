...
public class ComponentDetector
{
    ...
    private ComponentInfo? _divideComponentInfo;

    // 使用空间哈希字典进行快速查找，键为网格坐标，值为理论组件信息列表
    private readonly Dictionary<(int, int), List<ComponentInfo>> _spatialLookup = new();
    ...
    public List<ComponentResult> Detect(Mat binaryImage, List<ComponentInfo> allComponentInfos)
    {
        ...
        // 3. 查找最外层轮廓
        Cv2.FindContours(processedBinary, out var contours, out _, RetrievalModes.External,
    ...
    private void BuildSpatialLookup(List<ComponentInfo> allComponentInfos)
    {
        ...
        var cellSize = (int)_searchRadiusPixels;
        if (cellSize <= 0) cellSize = 100; // 防止除零错误

        foreach (var info in allComponentInfos)
        {
            var keyX = (int)(info.Center.X / cellSize);
            var keyY = (int)(info.Center.Y / cellSize);

            // 将组件信息加入其中心点所在的网格
            if (!_spatialLookup.ContainsKey((keyX, keyY))) _spatialLookup[(keyX, keyY)] = new List<ComponentInfo>();
        ...
        {
            var combinedBox = a11.BoundingBox.Union(a12.BoundingBox);
            _divideComponentInfo = new ComponentInfo
            {
                Id = "A11_A12_DIVIDE",
                Center = new Point2f(combinedBox.X + combinedBox.Width / 2.0f,
                    combinedBox.Y + combinedBox.Height / 2.0f),
    ...
    private List<ComponentResult> DivideAndProcessComponents(Mat binaryImage, Rect box, HashSet<string> detectedIds)
    {
        ...
        var start = (int)(roi.Width * 0.2);
        var end = (int)(roi.Width * 0.8);
        ...
        {
            // 处理左半部分
            var leftRect = new Rect(0, 0, splitX, roi.Height);
            ProcessDividedRoi(new Mat(roi, leftRect), box.TopLeft, detectedIds, newComponents);

            // 处理右半部分 (注意原点坐标需要偏移)
            var rightOrigin = new Point(box.TopLeft.X + splitX, box.TopLeft.Y);
            var rightRect = new Rect(splitX, 0, roi.Width - splitX, roi.Height);
    ...
    /// </summary>
    private void ProcessDividedRoi(Mat subRoi, Point originInGlobal, HashSet<string> detectedIds,
        List<ComponentResult> results)
    {
        using (subRoi) // 确保传入的Mat在此方法结束时被释放
        {
            ...
            Cv2.FindContours(subRoi, out var contours, out _, RetrievalModes.External,
                ContourApproximationModes.ApproxSimple);
            if (contours.Length > 0)
            {
                ...
                var box = Cv2.BoundingRect(largestContour);
                box.X += originInGlobal.X;
                box.Y += originInGlobal.Y;
                var globalContour = largestContour
                    .Select(p => new Point(p.X + originInGlobal.X, p.Y + originInGlobal.Y)).ToArray();
    ...
    ///     在指定的父组件区域内检测子组件。
    /// </summary>
    private List<ComponentResult> DetectChildComponents(Mat binaryImage, ComponentResult parent,
        HashSet<string> detectedIds)
    {
        var childComponents = new List<ComponentResult>();
        var roiRect = new Rect(parent.XMin, parent.YMin, parent.Width, parent.Height);
        ...
        foreach (var contour in contours)
        {
            ...
            if (child != null) childComponents.Add(child);
        }
    ...
    /// </summary>
    private List<ComponentResult> DetectRestComponents(Mat binaryImage, List<ComponentInfo> allComponentInfos,
        HashSet<string> detectedIds)
    {
        ...
        foreach (var info in missingInfos)
        {
            ...
            var searchBox = info.BoundingBox;
            ...
            // 2. 在小区域内查找轮廓
            Cv2.FindContours(region, out var contours, out _, RetrievalModes.External,
                ContourApproximationModes.ApproxSimple);
            ...
            var distance = center.DistanceTo(info.Center);
    ...
    {
        var center = new Point2f(box.X + box.Width / 2.0f, box.Y + box.Height / 2.0f);
        var keyX = (int)(center.X / _searchRadiusPixels);
        var keyY = (int)(center.Y / _searchRadiusPixels);

        ComponentInfo? bestMatch = null;
        var minDistance = _searchRadiusPixels; // 只有小于搜索半径的才算匹配
    ...
    /// </summary>
    private ComponentResult CreateComponentResult(ComponentInfo info, Rect box, Point2f center, double distancePixels,
        Point[] contour)
    {
