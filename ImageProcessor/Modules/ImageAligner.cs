// VisionLibrary/Modules/ImageAligner.cs
using OpenCvSharp;
...
namespace VisionLibrary.Modules
{
    ...
    public static class ImageAligner
    {
        ...
        public static Mat ExecuteAlignmentPipeline(Mat boardMat, Mat cadTemplateMat, Point2f[] sourceCorners, ImageInspectionConfig config)
        {
        ...
        private static Mat CorrectPerspective(Mat inputImage, Point2f[] corners)
        {
            ...
            double width = Math.Max(corners[1].DistanceTo(corners[0]), corners[2].DistanceTo(corners[3]));
            double height = Math.Max(corners[3].DistanceTo(corners[0]), corners[2].DistanceTo(corners[1]));
            int maxWidth = (int)Math.Round(width);
            int maxHeight = (int)Math.Round(height);
            ...
            // 计算透视变换矩阵并应用
            using Mat matrix = Cv2.GetPerspectiveTransform(corners, destinationPoints);
            Mat result = new Mat();
        ...
        {
            int targetWidthPx = (int)Math.Round(config.TargetBoardWidthMm / config.PixelSize);
            int targetHeightPx = (int)Math.Round(config.TargetBoardHeightMm / config.PixelSize);

            Mat result = new Mat();
            Cv2.Resize(inputImage, result, new Size(targetWidthPx, targetHeightPx), 0, 0, InterpolationFlags.Linear);
            return result;
        }
        ...
        private static Mat AlignToCad(Mat actualImage, Mat cadImage, ImageInspectionConfig config)
        {
            ...
            float cadTotalWidthMm = (float)(config.TargetBoardWidthMm + 2 * config.CuttingFrame);
            float cadTotalHeightMm = (float)(config.TargetBoardHeightMm + 2 * config.CuttingFrame);
            float pxPerMmX = cadImage.Width / cadTotalWidthMm;
            float pxPerMmY = cadImage.Height / cadTotalHeightMm;
            ...
            Point2f[] orderedCadPoints = { cadDestinationPoints[2], cadDestinationPoints[3], cadDestinationPoints[1], cadDestinationPoints[0] };

            // 5. 计算变换矩阵，将实际图像warp到CAD模板的目标区域
            using Mat matrix = Cv2.GetPerspectiveTransform(actualSourcePoints, orderedCadPoints);
            using Mat warpedActual = new Mat();
            Cv2.WarpPerspective(actualImage, warpedActual, matrix, cadImage.Size());

            // 6. 创建掩码，精确地将变换后的图像粘贴到CAD背景上
            using Mat mask = new Mat(cadImage.Size(), MatType.CV_8UC1, Scalar.Black);
            var cadPointsInt = orderedCadPoints.Select(p => new Point((int)p.X, (int)p.Y));
            Cv2.FillPoly(mask, new[] { cadPointsInt }, Scalar.White);

            Mat finalImage = cadImage.Clone();
