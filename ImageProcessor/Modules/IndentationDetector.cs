...
public class IndentationDetector
{
    ...
    public List<IndentationResult> Detect(Mat image, List<CadRawData> indentationData)
    {
        ...
        var lineWidth = _config.IndentationSampleLineWidth;
            ...
            switch (objType)
            {
                ...
                case "ARC":
                {
                        ...
                        row.StartAngle.HasValue && row.TotalAngle.HasValue)
                    {
                        ...
                        var startAngle = row.StartAngle.Value;
                        var totalAngle = row.TotalAngle.Value;
                        ...
                        result.AverageGray = CalculateShapeAverageGray(grayImage,
                            mask => Cv2.Ellipse(mask, circleCenter, axes, 0, startAngle, endAngle, Scalar.White,
                                lineWidth));
                    }
    ...
    {
        var x = (startPoint.X + endPoint.X) / 2.0;
        var y = (startPoint.Y + endPoint.Y) / 2.0;
    ...
    {
        // 计算圆弧的中间角度
        var midAngleRad = (startAngle + totalAngle / 2.0) * Math.PI / 180.0;
        // 计算中点坐标
        var x = center.X + radius * Math.Cos(midAngleRad);
        var y = center.Y + radius * Math.Sin(midAngleRad);
        return new Point2d(x, y);
    }
    ...
    private double CalculateShapeAverageGray(Mat grayImage, Action<Mat> drawAction)
    {
        ...
        // 使用掩码计算指定区域的平均值
        var averageScalar = Cv2.Mean(grayImage, mask);
