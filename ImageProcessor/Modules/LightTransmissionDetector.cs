// VisionLibrary/Modules/LightTransmissionDetector.cs
using OpenCvSharp;
...
namespace VisionLibrary.Modules
{
    ...
    /// </summary>
    public class LightTransmissionDetector
    {
        private readonly ImageInspectionConfig _config;
        private readonly CoordinateConverter _converter;

        public LightTransmissionDetector(ImageInspectionConfig config, CoordinateConverter converter)
        {
            _config = config;
        ...
        public List<LightTransmissionResult> Detect(Mat backlitImage, List<CadRawData> cadData)
        {
            ...
            Parallel.For(0, validEntities.Count, i =>
            {
                ...
                var (samples, normals) = SampleEntity(csvRow, _config.TransmissionSampleStep);

                for (int j = 0; j < samples.Count; j++)
                {
                    var sampleResult = ProcessTransmissionSamplePoint(grayImage, samples[j], normals[j], i + 1, csvRow.ObjectType);
        ...
        /// 处理单个采样点，检查其横截面的透光性。
        /// </summary>
        private LightTransmissionResult ProcessTransmissionSamplePoint(Mat grayImage, Point2d designPointMm, (double nx, double ny) normal, int curveId, string curveType)
        {
            ...
            double checkHalfWidth = _config.TransmissionCheckWidthPixels / 2.0;

            // 迭代步长可以小于1，以确保在倾斜时不会跳过像素
            for (double step = -checkHalfWidth; step <= checkHalfWidth; step += 0.5)
            {
                double currentX = mappedPixel.X + step * normal.nx;
                double currentY = mappedPixel.Y + step * normal.ny;
            ...
            // 这种方法比判断平均值更严格、更可靠。
            if (result.MinGrayValue >= _config.MinTransmissionGrayValue)
            {
